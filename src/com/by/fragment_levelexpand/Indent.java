...
import com.by.adapter.IndentAdapter;
...
public class Indent extends Fragment implements OnClickListener,
	...
	public void onAttach(Activity activity) {
		...
		mActivity = activity;
		Config.ITEM_POSITION = 22;
		// Log.e("TAG", "ITEM_POSITION" + Config.ITEM_POSITION);
	}
		...
		if (NetworkUtil.isNetworkAvailable(mActivity)) {
			...
			utils.send(HttpMethod.GET, urlPath, new RequestCallBack<String>() {
				...
				public void onFailure(HttpException arg0, String arg1) {
					...
					LoginUtils.onLogin(mActivity, false, 0);
				}
				...
				public void onSuccess(ResponseInfo<String> arg0) {
					...
					if ("-1".equals(json)) {
						...
						if (adapter == null) {
							adapter = new IndentAdapter(cList, mActivity);
	...
	public void jsonParse(String json) throws JSONException {
		...
		cList = new ArrayList<OrderBean>();

		// if (array.length() == 0 && isQuery) {
		if (array.length() == 0) {
			// Toast.makeText(getActivity(), "无订单信息...", 0).show();
			empty.setText("亲,您最近无订单信息。。。");
			list.clear();
			adapter = null;
		} else {
			cList = JSON.parseArray(json, OrderBean.class);
	...
	public void onItemClick(AdapterView<?> parent, View view, int position,
		...
		int oid = bean.getOid();
		Config.ORDERTAIL_OID = oid;// 使之变为全局变量；
		//
		...
		// args.putString("title", title);区分买家和卖家
